...
#---- 
	...
	RowSum1:
		...
		beqz $s3, firstRSum 			#after first sum check everyother row sum 
			beq $s4, $t3, firstRSum
				la $s2, no_msg
		firstRSum:
	...
	bge $s3, $s1, endCS1
		...
		beq $s4, $t3, CheckCSum 		#check if  column sum matches other sums
			la $s2, no_msg
		CheckCSum:
	...
	firstD:
		...
		beq $s4, $t3, CheckD1Sum 		#check if  diagonal sum matches other sums
			la $s2, no_msg
		CheckD1Sum:
	...
	secondD:
		...
		beq $s4, $t3, CheckD2Sum 		#check if  diagonal sum matches other sums
			la $s2, no_msg
		CheckD2Sum:
